# -*- coding: utf-8 -*-
"""score 分带 fwd15/fwd20，验证分数与收益是否非单调。"""
import time
...
bands = defaultdict(lambda: {"f15": [], "f20": []})
for di, date in enumerate(axis):
    ...
    for code, s in series_map.items():
        ...
        b = f"{sc:.0f}"
        bands[b]["f15"].append(f15)
        bands[b]["f20"].append(f20)

print("\nscore 分带:")
print(f"{'band':6s} {'n':6s} {'fwd15':8s} {'win15':6s} {'fwd20':8s} {'win20':6s}")
for b in sorted(bands, key=float):
    d = bands[b]
    ...
    print(f"{b+'~'+str(int(b)+1):6s} {len(d['f15']):6d} {a15:8.2f} {w15:6.1f} {a20:8.2f} {w20:6.1f}")
